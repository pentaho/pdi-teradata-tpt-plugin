...
public class TeraDataBulkLoaderRoutines {
	...
	private String createInsertCommand() {
		...
		for (int i=0; i < fieldTable.length; i++){
			...
			if (i < fieldTable.length - 1) { cmd.append(","); }
			cmd.append("\n");
		}
		cmd.append("   )\n");
		cmd.append(" VALUES\n");
		...
		for (int i=0; i < fieldStream.length; i++){
			...
			if (i < fieldStream.length - 1) { cmd.append(","); }
			cmd.append("\n");
		}
		cmd.append("   )\n");
	...
	private String paren(String s) { return "(" + s + ")"; }
	...
	public String createGeneratedScriptFile() throws Exception {
		...
		for (int i=0; i < parent.getInputRowMeta().size(); i++){
			inputFieldTypes.put (parent.getInputRowMeta().getValueMeta(i).getName(),parent.getInputRowMeta().getValueMeta(i).getType());
			inputFieldLength.put(parent.getInputRowMeta().getValueMeta(i).getName(),parent.getInputRowMeta().getValueMeta(i).getLength());
		}
	...
	public String createGeneratedScriptFile(Map<String, Integer> inputFieldTypes, Map<String, Integer> inputFieldLength) throws Exception {
		...
		DefineSchema dataConnector = new DefineSchema("ACCESS_MODULE_READER","DATACONNECTOR PRODUCER",this.meta.getSchemaName());
        ...
        if (this.meta.getDropLogTable())   { dropClauses.addClause(quote(dropTable(this.meta.getLogTable()   ,""))); 	drop = true; }
        if (this.meta.getDropWorkTable())  { dropClauses.addClause(quote(dropTable(this.meta.getWorkTable()  ,"WT")));  drop = true; }
        if (this.meta.getDropErrorTable()) { dropClauses.addClause(quote(dropTable(this.meta.getErrorTable() ,"ET")));  drop = true; }
        if (this.meta.getDropErrorTable2()){ dropClauses.addClause(quote(dropTable(this.meta.getErrorTable2(),"UV")));	drop = true; }
        if (drop){
        ...
        case 0:
        	...
        	loadCommand = createStep("Load_Table",cmdApply.toString(), toSelectOperator("UPDATE_OPERATOR[2]","ACCESS_MODULE_READER[2]"));
        	break;
        case 1:
        	cmdApply.addClause(createUpsertCommand());
        	loadCommand = createStep("Upsert_Table",cmdApply.toString() + addMissingOptions(), toSelectOperator("UPDATE_OPERATOR[2]","ACCESS_MODULE_READER[2]"));
        	break;
        }

        String script = teradataJob(
        		BaseMessages.getString(PKG, "TeraDataBulkLoaderDialog.Script.Name") , //"JobName_Goes_Here", 
        		BaseMessages.getString(PKG, "TeraDataBulkLoaderDialog.Script.Description") , // "Description goes here", 
        		(tableSchema   != null ? tableSchema.toString()   : "") + 
         		(ddlOptions    != null ? ddlOptions.toString() : "") + 
         		(dataConnector != null ? dataConnector.toString() : "") + 
        		(updateOptions != null ? updateOptions.toString() : "") + 
        		(dropTables    != null ? dropTables : "") +
        		loadCommand);
